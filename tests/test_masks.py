...
class TestMaskedParameters:
    ...
    def test_wrong_mode(self):
        mp = self._create_masked_parameter(perturbation_axis=PerturbationAxis.RANDOM)
        with pytest.raises(NotImplementedError):
            mp.perturb(mode=10)

    @pytest.mark.parametrize("perturbation", list(PerturbationAxis))
    def test_perturbation(self, perturbation):
        mp = self._create_masked_parameter(perturbation_axis=perturbation)
        factor = 1
    ...
    @pytest.mark.parametrize("perturbation", list(PerturbationAxis))
    def test_negative_perturbation(self, perturbation):
        mp = self._create_masked_parameter(perturbation_axis=perturbation)
        with pytest.raises(AssertionError):
            mp.perturb(-1)

    @pytest.mark.parametrize("perturbation", list(PerturbationAxis))
    def test_perturbation_remove_add(self, perturbation):
        mp = self._create_masked_parameter(perturbation_axis=perturbation)
        factor = 1
        if perturbation != PerturbationAxis.RANDOM:
            factor = 3
    ...
    def test_perturbation_mode(self, perturbation, mode):
        mp = self._create_masked_parameter(perturbation_axis=perturbation)
    ...
    @pytest.mark.parametrize("perturbation", list(PerturbationAxis))
    def test_perturbation_invert_remove(self, perturbation):
        mp = self._create_masked_parameter(perturbation_axis=perturbation)
    ...
    @pytest.mark.parametrize("perturbation", list(PerturbationAxis))
    def test_perturbation_add_remove(self, perturbation):
        mp = self._create_masked_parameter(perturbation_axis=perturbation)
        factor = 1
        if perturbation != PerturbationAxis.RANDOM:
            factor = 3
    ...
    def _create_masked_parameter(self, perturbation_axis: PerturbationAxis):
